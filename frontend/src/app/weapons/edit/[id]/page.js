...
export default function EditWeaponPage({ params }) {
  ...
  useEffect(() => {
    fetch(`${apiUrl}/weapon/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setWeapon({
          ...data,
          Attack: (data.attack || []).join(", "),
          Guard: (data.guard || []).join(", "),
          Scalling: (data.scalling || []).join(", "),
  ...
  async function handleSubmit(e) {
    ...
    const res = await fetch(`${apiUrl}/weapon/${id}`, {
      method: "PUT",
  ...
  if (!weapon) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-zinc-900 to-zinc-800">
        <div className="text-red-400 text-lg">{error || "Weapon not found."}</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-zinc-900 to-zinc-800 flex items-center justify-center p-10">
      <div className="bg-zinc-950 border border-zinc-700 shadow-2xl p-10 w-[900px] max-w-full rounded-2xl transition-all duration-300">
        <h1 className="text-[#bfa046] text-3xl font-extrabold mb-4 drop-shadow-lg">Edit Weapon | Elden Ring Wiki</h1>
